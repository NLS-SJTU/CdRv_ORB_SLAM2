...
vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int &w)
{
    ...
    // 从mvOrderedWeights找出第一个大于等于w的那个迭代器
    // 由于mvOrderedWeights为降序排列，comp改为a>b，使用upper_bound()返回第一个大于等于w；使用lower_bound()返回第一个大于w；
...
set<MapPoint*> KeyFrame::GetMapPoints()
{
    ...
    for(size_t i=0, iend=mvpMapPoints.size(); i<iend; i++)
    {
        ...
        //判断三维点是否为坏点，MapPoints::SetBadFlag(),keyframe::SetBadFlag();
        if(!pMP->isBad())
            s.insert(pMP);
    }
    return s;
