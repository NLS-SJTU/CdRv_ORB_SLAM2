...
void Tracking::SetLocalMapper(LocalMapping *pLocalMapper)   // 設置指向其他線程的指針
{
...
void Tracking::Track()
{
    ...
    else// 步骤2：跟踪
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(!mbVO)
                {
                    ...
                    if(!mVelocity.empty())
                    {
                        bOK = TrackWithMotionModel();   // 不加後續判斷，那麼MM不好的時候就直接到後面變成LOST,進行重定位
                        // 这个地方是不是应该加上：
                        // if(!bOK)
                        //    bOK = TrackReferenceKeyFrame();
                    }
        ...
        // 跟踪失败，并且relocation也没有搞定，只能重新Reset  清空地圖，關鍵幀等系統數據
        if(mState==LOST)
        {
